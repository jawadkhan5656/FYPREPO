        ...
        await connection.query(select, async(err, results, fields) => {
            if(err)
                ...
                if(results.length > 0)
                    ...
                    VALUES ((SELECT program_id FROM program WHERE program_name = ?), (SELECT cs_id FROM current_session WHERE springFall = ? AND year = ?), (SELECT department_id FROM department WHERE department_name = ?), (SELECT section_id FROM section WHERE section_name = ?), (SELECT teacher_id FROM teacher WHERE teacher_name = ?), (SELECT av.av_id FROM available_rooms av JOIN room r ON av.room_id = r.room_id WHERE r.room_number = ? AND r.department = ?), (SELECT os.os_id FROM offered_subjects os JOIN course c ON os.course_id = c.course_id WHERE c.course_name = ?),(SELECT session_id FROM session WHERE session_name = ? AND batch = ?), ?, ?, ?, ?, ?)
                `;
